...
- 常用来代替arguments(arguments.callee指函数本身)
...
- 拓展运算符
  ...
  - 能够将``数组``转换为逗号分隔的``参数序列 ``
    ...
    ```javascript
    const into1 = ["mika","patric","linmo","santa"]
    function wajijiwa(){
        console.log(arguments)
    }
    wajijiwa(into1) // array
    wajijiwa(...into1)//"mika","patric","linmo","santa"
    ```
...
还是以`Animal` 和 `Rabbit` 为例，要查找 `rabbit.run` 方法，JavaScript 引擎会进行如下检查：
